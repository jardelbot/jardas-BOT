...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nSelamat datang ao grupo *${mdata.subject}*\nLeia as regras na descrição!`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["0@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'fig':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`enviar imagem com legenda ${prefix}sticker ou resposta de imagem que foi enviada`)
					}
					break
				case 'gtts':
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Preciso de um nome para pesquisar, cadê?', text, {quoted: mek})
                ...
                case 'ban':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Entendido, Removendo o Membro : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Digite /Welcome 1 Para ativar e /Welcome 0 para desativar. Por Padrão, é 0')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ativado Com Sucesso ✔')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Já está Ativo, Amor')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativou com sucesso o recurso de boas-vindas neste grupo✔️')
					} else {
						reply('Comando Desativado, caso queira ativar, digite /welcome 1')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
